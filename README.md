...
> [!TIP]
> ### 💡 연구 아이디어 — 무게중심(Centroid)으로 주도권 파악하기
>
> **기본 원리**
> 두 선수 좌표의 중점(centroid)이 어느 쪽으로 이동하는지를 보면,
> 누가 상대를 밀어붙이고 있는지 알 수 있습니다.
>
> **계산 방법**
> ```python
> centroid_x = (x_m_1 + x_m_2) / 2
> centroid_y = (y_m_1 + y_m_2) / 2
> ```
>
> **계산 예시**
>
...
> | 1259  | 4.153     | 4.876     |
>
> `centroid_x`가 선수1 쪽에서 선수2 쪽으로 이동하면 → **선수1이 밀리고 있는 것**이고,
> 반대면 **선수1이 전진 압박 중**인 겁니다.
>
> ---
>
> **더 실용적인 주도권 지표 3가지**
>
> **① 전진 / 후퇴 비율**
> 각 선수가 상대 방향으로 이동한 프레임 수를 세면 됩니다.
> ```python
> # 선수1의 전진 여부: 상대(선수2)와의 거리가 줄어드는지
> dist = sqrt((x_m_1 - x_m_2)² + (y_m_1 - y_m_2)²)
> # dist가 감소하면 전진, 증가하면 후퇴
> ```
> 전진 비율이 높은 선수가 주도권을 가진 것으로 판단합니다.
>
> **② 경기장 중앙 점유율**
> 경기장 중심 좌표를 기준으로, 누가 더 중앙에 가까이 위치하는지를 시간 비율로 측정합니다.
> 중앙을 차지한 선수가 공간적 우위를 가진 것입니다.
>
> **③ 시계열 구간별 주도권 변화 추적**
> centroid 이동 방향에 일정 시간 윈도우(예: 5초)로 이동 평균을 적용하면,
> 라운드 내에서 주도권이 언제 바뀌는지 흐름이 보입니다.
