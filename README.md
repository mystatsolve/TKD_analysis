...
#### 1. 이동 속도 및 가속도
프레임 간 좌표 변화와 시간 간격으로 순간 속도(m/s)·가속도(m/s²)를 계산합니다.
...
→ "이 선수는 공격 진입 시 평균 2.3m/s, 최대 가속도 4.1m/s²" 수준의 수치를 산출합니다.

#### 2. 방향전환(COD) 능력 평가
이동 벡터의 각도 변화율로 방향전환 이벤트를 감지합니다.
...
→ 방향전환 빈도(회/분), 전환 시 속도, 시간대별 분포를 산출합니다.

#### 3. 이동 효율성
실제 이동 거리 대비 직선 거리의 비율로 움직임의 효율성을 측정합니다.

```
이동 효율성(%) = 직선 거리 / 실제 이동 거리 × 100
```

→ 값이 낮을수록 불필요한 왕복·지그재그가 많음을 의미합니다.

#### 4. 라운드별 운동 부하 (Work Rate)
시간 구간(라운드) 단위로 운동 강도를 산출합니다.
...
→ 축구의 GPS 기반 총 이동거리·스프린트 분석과 동일한 개념입니다.

#### 5. 무게중심(Centroid) 이동 패턴
두 선수의 좌표를 결합하여 경기 전체의 중심점 궤적을 시각화합니다.

```
무게중심 = (Player1 좌표 + Player2 좌표) / 2
```

→ 무게중심이 한쪽으로 치우칠수록 해당 선수가 경기 공간을 주도하고 있음을 나타냅니다.
